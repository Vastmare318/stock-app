...
st.caption("東証33業種・全銘柄連携データ ｜ 主要8銘柄対応 ｜ 朝夜ニュース完全網羅")

# ==========================================
# 主要銘柄のマスター辞書（主要8銘柄 ＋ 日本アクア・中村超硬など）
# ==========================================
MASTER_STOCK_INFO = {
...
with tab1:
    ...
    if st.button("🔍 診断を実行する", type="primary", key="tab1_btn"):
        if not target_code.isdigit() or len(target_code) != 4:
            st.error("⚠️ 証券コードは**4桁の数字**で入力してください（例: 8306、7203など）。")
        else:
...
# ==========================================
# タブ2：マイ・ポートフォリオ診断（主要8銘柄一括）
# ==========================================
with tab2:
    st.subheader("🎯 マイ・ポートフォリオ一括診断（主要監視8銘柄）")
    st.write("夜のニュースでもおなじみの主要8銘柄（三菱UFJ、三菱商事、NTT、東京海上、JT、三菱HCキャピタル、積水ハウス、トヨタ）の最新状況を一括スキャンします。")
    
    my_portfolio_codes = ["8306", "8058", "9432", "8766", "2914", "8593", "1928", "7203"]
    
    if st.button("🚀 主要8銘柄を一括診断する", type="primary", key="tab2_btn"):
        portfolio_data = []
        alert_messages = []
        progress_bar = st.progress(0)
        status_text = st.empty()
        
        total_stocks = len(my_portfolio_codes)
        for i, code in enumerate(my_portfolio_codes):
            master = MASTER_STOCK_INFO.get(code, {})
            name = master.get("name", code)
            sector = master.get("sector", "不明")
            
            status_text.text(f"取得中... ({i+1}/{total_stocks}) {name}")
        ...
        status_text.text("✨ 全8銘柄の診断が完了しました！")
...
# ==========================================
with tab6:
    st.subheader("🌅 毎朝の経済ニュース要約")
    st.write("本日の株式相場に影響を与えそうな最新の経済ニュースをピックアップしてお届けします。")
    st.markdown("---")

    st.markdown("### 1. アメリカの「長〜いおカネの金利」が高くなったよ")
    st.markdown("""
    * アメリカで、国におカネを貸したときにもらえる利息（長期金利）が大きく上がって、約3年ぶりの高い水準になったんだ。
    * モノの値段が上がり続けていることや、国の借金の心配があることが原因で、みんなが国債を売ったのが理由だよ。
    * 金利が上がると、お家を買うときのローンや会社の活動にお金がかかりやすくなるので、世界の株式市場が少しドキドキしているんだ。
    """)

    st.markdown("### 2. 人工知能（AI）にかける期待の風向きが変わってきたよ")
    st.markdown("""
    * 最近の株式市場で大人気だったAI関連の大きな会社の株が、少し下がっているんだ。
    * 「AIをもっとすごいものにするのには、少し時間がかかるかも？」という心配や、関連企業の発言がきっかけになったよ。
    * でも、市場全体で見ると、AI以外の他の会社の株がしっかり買われているところもあり、みんなの注目するものが変わりつつあるよ。
    """)

    st.markdown("### 3. 日本のこれからの「金利」や「政治の動き」に注目が集まっているよ")
    st.markdown("""
    * 日本でも、これから銀行の金利が上がるかどうかの大切な会議がひかえていて、みんながソワソワしているよ。
    * お金の値段が変わるかもしれない大切な時期なので、日本の株や円の値段（為替）がどう動くか、大人の人たちがとても気にとめているんだ。
    """)
    
    st.markdown("=" * 50)
    
    st.subheader("🌙 毎晩の主要8銘柄レポート（夜のニュース）")
    st.write("毎晩20時に確認している、監視対象の主要8銘柄（三菱UFJ、三菱商事、NTT、東京海上、JT、三菱HCキャピタル、積水ハウス、トヨタ）の重要ポイントをおさらいできます。")
    st.markdown("---")

    st.markdown("### 🏦 1. 銀行・商社・通信（三菱UFJ・三菱商事・NTT）")
    st.markdown("""
    * **三菱UFJ**: 日本の金利の動きや、アメリカの金融市場の変化がダイレクトに影響する大黒柱。金利が上がると追い風になりやすいよ。
    * **三菱商事**: 世界中の資源やビジネスを手がけているから、海外の景気や資源（エネルギーなど）の値段のニュースを要チェック！
    * **NTT**: 安定した配当が魅力の超大型株。通信の規制や次世代のネットワーク技術に関する話題に注目。
    """)

    st.markdown("### 🛡️ 2. 保険・タバコ・リース（東京海上・JT・三菱HCキャピタル）")
    st.markdown("""
    * **東京海上**: 世界中で自然災害（ハリケーンや地震など）が大きいと、保険金の支払いがニュースになるので、海外のニュースは要チェック。
    * **JT**: 世界をまたにかける高配当の代名詞。為替（円高・円安）や海外のタバコ市場のルール変更などが夜のニュースの話題になりやすいよ。
    * **三菱HCキャピタル**: 連続増配が続く大人気リース株。景気の波を受けにくいけれど、金利のアップダウンには注意が必要。
    """)

    st.markdown("### 🏠🚗 3. 住宅・自動車（積水ハウス・トヨタ）")
    st.markdown("""
    * **積水ハウス**: 日本だけでなくアメリカの住宅市場の動きも大きなお仕事。アメリカの住宅ローン金利が夜のニュースでどう言われているかがカギ！
    * **トヨタ**: 日本のモノづくりを引っ張る大看板。円安・円高の為替のニュースや、世界の自動車（電気自動車など）の売れ行きが毎日大注目されるよ。
    """)
    
    st.markdown("---")
    st.info("💡 朝のニュースで世界や日本の風向きを知り、夜のニュースで主要8銘柄の状況を確認して、どっしり構えた投資を続けよう！")
